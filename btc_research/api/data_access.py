...
class DataAccess:
        ...
        try:
            ...
            async with self.strategy_manager.get_strategy_runner(strategy_id) as runner:
                ...
                for trade in all_trades:
                    ...
                    trade_dict[
                        "realized_pnl"
                    ] = None  # TODO: Implement proper P&L calculation
        ...
        try:
            ...
            async with self.strategy_manager.get_strategy_runner(strategy_id) as runner:
                ...
                for position in positions.values():
                    ...
                    pos_dict["market_price"] = str(
                        paper_trader.get_current_price(position.symbol) or 0.0
                    )
    ...
    async def get_strategy_orders(
        ...
        try:
            ...
            async with self.strategy_manager.get_strategy_runner(strategy_id) as runner:
                ...
                if status_filter:
                    all_orders = [
                        o for o in all_orders if o.status.value == status_filter
                    ]
                ...
                for order in all_orders:
                    ...
                    order_dict["type"] = order_dict.pop(
                        "order_type"
            ...
            # Fall back to historical data
            return await self._get_historical_orders(
                strategy_id, skip, limit, status_filter
            )
    ...
    async def get_strategy_performance(
        self, strategy_id: str, period: str = "1d"
    ) -> Optional[dict[str, Any]]:
        """
        ...
        try:
            ...
            # Calculate P&L over time using a simple running total.  This also
            # doubles as our balance history for performance calculations.
            pnl_history: list[dict[str, Any]] = []
            cumulative_pnl = 0.0

            if trades:
                for trade in sorted(trades, key=lambda t: t["timestamp"]):
                    trade_pnl = -float(trade.get("commission", 0))
                    cumulative_pnl += trade_pnl

                    pnl_history.append(
                        {
                            "timestamp": trade["timestamp"],
                            "balance": stats.get("total_balance", 10000)
                            + cumulative_pnl,
                        }
                    )
            ...
            # Calculate metrics using the PerformanceCalculator for a more
            # realistic view of live trading performance.
            calculator = PerformanceCalculator()
            metrics = calculator.calculate_metrics(
                trades=trades,
                balance_history=pnl_history,
                initial_balance=stats.get("initial_balance", 10000),
                current_balance=current_balance + current_unrealized,
            )
    ...
    async def _get_historical_statistics(
        self, strategy_id: str
    ) -> Optional[dict[str, Any]]:
    ...
    async def _get_historical_trades(
        self, strategy_id: str, skip: int, limit: int, symbol: Optional[str] = None
    ) -> PaginationResult:
                ...
                for line in f:
                    try:
                        ...
                        if (
                            ...
                            or order.get("status") == status_filter
                        ):
                            orders.append(order)
                    except json.JSONDecodeError:
                        continue
    ...
    async def get_strategy_condition_metrics(
        self, strategy_id: str
    ) -> Optional[dict[str, Any]]:
        """
        ...
        try:
            ...
            async with self.strategy_manager.get_strategy_runner(strategy_id) as runner:
                if hasattr(runner, "get_condition_metrics"):
                    ...
                    logger.warning(
                        f"Strategy runner for {strategy_id} does not have get_condition_metrics method"
                    )
                    raise AttributeError(
                        "'StrategyRunner' object has no attribute 'get_condition_metrics'"
                    )

        except Exception as e:
            logger.warning(
                f"Could not get condition metrics for active strategy {strategy_id}: {e}"
            )

            # Fall back to historical data if available
            return await self._get_historical_condition_metrics(strategy_id)

    async def get_strategy_condition_summary(
        self, strategy_id: str
    ) -> Optional[dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.warning(
                f"Could not get condition summary for active strategy {strategy_id}: {e}"
            )

            # Fall back to historical data if available
            return await self._get_historical_condition_summary(strategy_id)

    async def _get_historical_condition_metrics(
        self, strategy_id: str
    ) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get historical condition metrics for {strategy_id}: {e}"
            )
            return None

    async def _get_historical_condition_summary(
        self, strategy_id: str
    ) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get historical condition summary for {strategy_id}: {e}"
            )
            return None
        ...
        try:
            ...
            async with self.strategy_manager.get_strategy_runner(strategy_id) as runner:
                ...
                try:
                    ...
                    metrics_file = (
                        self.data_dir / f"{strategy_id}_condition_metrics.json"
                    )
                    summary_file = (
                        self.data_dir / f"{strategy_id}_condition_summary.json"
                    )
                    ...
                    logger.warning(
                        f"Failed to save condition metrics for {strategy_id}: {e}"
                    )
