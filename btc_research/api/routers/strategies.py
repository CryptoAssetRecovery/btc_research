...
from ..dependencies import RequiredAuth, get_strategy_manager
from ..models import (
    BaseResponse,
...
async def start_strategy(
    ...
    try:
        ...
        formatted_info = {
            ...
            "current_balance": strategy_info.get("trading", {}).get(
                "total_equity", strategy_info["initial_balance"]
            ),
            "total_trades": strategy_info.get("trading", {}).get("total_trades", 0),
            "active_positions": strategy_info.get("trading", {}).get(
                "total_positions", 0
            ),
        }

        logger.info(
            f"Started strategy {strategy_id[:8]} for user {current_user['user_id']}"
        )

        return StrategyResponse(
            message="Strategy started successfully", strategy=formatted_info
        )
...
async def stop_strategy(
    ...
    try:
        ...
        logger.info(
            f"Stopped strategy {strategy_id[:8]} for user {current_user['user_id']}"
        )

        return BaseResponse(message=f"Strategy {strategy_id} stopped successfully")
...
async def list_active_strategies(
    ...
    try:
        ...
        strategies_data = await strategy_manager.get_active_strategies(
            user_id=current_user["user_id"]
        )
        ...
        logger.info(
            f"Listed {len(formatted_strategies)} active strategies for user {current_user['user_id']}"
        )
...
async def get_strategy_status(
    ...
    try:
        ...
        return StrategyResponse(
            message="Strategy status retrieved successfully", strategy=formatted_info
        )
...
async def get_strategy_statistics(
    ...
    try:
        ...
        # Calculate additional metrics based on balance information returned
        # by the PaperTrader. Older versions of the API expected fields like
        # ``total_pnl`` and ``realized_pnl`` but these are not provided by
        # ``get_performance_stats``.  Instead we derive them from the available
        # balance data to provide more accurate statistics in the web UI.

        initial_balance = trading_stats.get(
            "initial_balance",
            strategy_info.get("initial_balance", 10000.0),
        )

        total_balance = trading_stats.get("total_balance", initial_balance)
        unrealized_pnl = trading_stats.get("total_unrealized_pnl", 0.0)
        total_equity = trading_stats.get("total_equity", total_balance + unrealized_pnl)
        ...
        win_rate = 0.0
        if (
            "performance" in strategy_stats
            and "signals_generated" in strategy_stats["performance"]
        ):
            signals = strategy_stats["performance"]["signals_generated"]
            successful_orders = strategy_stats["performance"].get(
                "successful_orders", 0
            )
        ...
        formatted_stats = {
            ...
            "performance": {
                ...
                "win_rate": win_rate,
                "signals_generated": strategy_stats.get("performance", {}).get(
                    "signals_generated", 0
                ),
            ...
            "trading": {
                ...
                "available_balance": trading_stats.get(
            ...
            "updated_at": strategy_stats.get(
                "last_update_time", strategy_info.get("updated_at")
            ),
        }
        ...
        return StrategyStatsResponse(
            message="Strategy statistics retrieved successfully", stats=formatted_stats
        )
...
async def get_strategy_details(
    ...
    try:
        ...
        return StrategyResponse(
            message="Strategy details retrieved successfully", strategy=formatted_info
        )
